...
case "$mode" in
    r2n)
        default_ref_filelist=2_synthesis/1_input/filelists/${module}.formality_r2n.f
        if [[ ! -f "$default_ref_filelist" ]]; then
            default_ref_filelist=2_synthesis/1_input/filelists/${module}.dft.f
        fi
        if [[ "${FM_USE_DWROOT:-0}" == "1" ]]; then
            default_ref_filelist=2_synthesis/1_input/filelists/${module}.formality_r2n_dwroot.f
        fi
        export REF_FILELIST=${REF_FILELIST:-$default_ref_filelist}
        ...
        export FM_DWROOT=${FM_DWROOT:-}
        if [[ "${FM_USE_DWROOT:-0}" == "1" ]]; then
            export FM_DWROOT=${FM_DWROOT:-/tools/synopsys/syn/W-2024.09-SP5-5}
        fi
        export DW_VERILOG_FILES=${DW_VERILOG_FILES:-""}
